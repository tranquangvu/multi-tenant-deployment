...
- **`scripts/`** — Deployment and helpers:
  ...
  - `py/tenant-network-ssm-params.py` — Emit JSON array of CloudFormation parameters for SSM paths (used by `deploy-tenant.sh`).
...
└── scripts/
    ...
    └── py/tenant-network-ssm-params.py
```
...
3. **Deploy root stack** for one (tenant, environment). Use **`deploy-tenant.sh`** so VPC and subnet SSM paths are merged from the registry. Calling **`deploy-stack.sh`** with only `params.json` will **omit** those parameters and the deploy will fail.
   ...
   **Manual `deploy-stack.sh`:** build a merged params file first, then deploy (same merge `deploy-tenant.sh` uses):
   ...
   NET="$(python3 ./scripts/py/tenant-network-ssm-params.py ./config/tenant-registry.yaml base staging)"
...
```text
                         +-----------------------------------+
                         | Landing Zone Networking (SSM)     |
                         |  VPC + Subnet IDs from SSM paths  |
                         |  e.g. /accelerator/network/vpc/*  |
                         +------------------+----------------+
                                            |
                                            v

===============================================================================
BASE ACCOUNT (TenantId=base)
===============================================================================
  Root stack mt-base-staging
    - SecurityStack: App SG + Db SG (in VPC)
    - AlbStack: ALB + TargetGroups (foo:8080, baz:8080) in PublicSubnet*
    - EcsClusterStack: ECS Cluster
    - RDS + ECS wiring:
        foo-app DB (PrivateSubnet*)  --> ECS Service foo-app (Fargate)
        baz-app DB (PrivateSubnet*)  --> ECS Service baz-app (Fargate)

  Root stack mt-base-production
    - same wiring as mt-base-staging

===============================================================================
ABC ACCOUNT (TenantId=abc)
===============================================================================
  Root stack mt-abc-production
    - SecurityStack: App SG + Db SG (in VPC)
    - AlbStack: ALB + TargetGroups (foo:8080, baz:8080) in PublicSubnet*
    - EcsClusterStack: ECS Cluster
    - RDS + ECS wiring:
